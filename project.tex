...
If we have a rule containing \textit{a for x in y}, where y is a list variable, we cannot apply the three rules explained above.
To solve this, we added another rule to translate the list variable y back to the list literal format.
...
\begin{mathpar}
    \inferrule*[right={$[\text{comp}]$}]
        {[] \vdash <\text{l := 1:2:[]}, s_{\perp, \perp}> \rightarrow [] \vdash s_{1:2[], \perp}, \quad [] \vdash <\text{y := [x + 1 for x in l]}, s> \rightarrow []? \vdash s_{1:2:[], 2:3:[]}}
