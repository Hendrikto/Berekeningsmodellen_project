...
\begin{mathpar}
    \inferrule*[right={$[\text{comp}]$}]
        {\inferrule*[right={$[\text{ass}_{ll}]$}]
            {}
            {<\text{S1}, s_0> \rightarrow s_1}
        \inferrule*[right={$[\text{lcomp}_{var}]$}]
            {\inferrule*[right={$[\text{ass}_{lc}]$}]
                {\inferrule*[right={$[\text{ass}_{ll}]$}]
                    {}
                    {<\text{y := 2:4:[]}, s_1> \rightarrow s_2}
                }
                {<\text{S2'}, s_1> \rightarrow s_2}
            }
            {<\text{S2}, s_1> \rightarrow s_2}
        }
        {<\text{S1; S2}, s_0> \rightarrow s_2}
\end{mathpar}

Because

$\begin{aligned}
    & \fancybr{\CC}{\mono{[a * 2 for a in 1:2:3:[] if b]}} s_1\\
    =\ & \fancybr{\AA}{\mono{a * 2}} s_1[\mono{a} \mapsto \fancybr{\NN}{\mono{1}}] \mono{:} \fancybr{\CC}{\mono{[a * 2 for a in 2:3:[] if b]}} s_1
    & \text{because } \fancybr{\BB}{\mono{b}} s_1[\mono{a} \mapsto \fancybr{\NN}{\mono{1}}] = \mathbf{tt}\\
    =\ & \mono{2:} \fancybr{\CC}{\mono{[a * 2 for a in 2:3:[] if b]}} s_1\\
    =\ & \mono{2:} \fancybr{\AA}{\mono{a * 2}} s_1[\mono{a} \mapsto \fancybr{\NN}{\mono{2}}] \mono{:} \fancybr{\CC}{\mono{[a * 2 for a in 3:[] if b]}} s_1
    & \text{because } \fancybr{\BB}{\mono{b}} s_1[\mono{a} \mapsto \fancybr{\NN}{\mono{2}}] = \mathbf{tt}\\
    =\ & \mono{2:4:} \fancybr{\CC}{\mono{[a * 2 for a in 3:[] if b]}} s_1\\
    =\ & \mono{2:4:} \fancybr{\CC}{\mono{[a * 2 for a in [] if b]}} s_1
    & \text{because } \fancybr{\BB}{\mono{b}} s_1[\mono{a} \mapsto \fancybr{\NN}{\mono{3}}] = \mathbf{ff}\\
    =\ & \mono{2:4:[]}\\
\end{aligned}$
