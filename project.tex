...
\begin{mathpar}
    ...
    \inferrule*[left={$[\text{while}^{ff}$]}, right={if $\fancybr{\BB}{b}s = \mathbf{ff}$}]
        {}
        {<\mono{while } b \mono{ do } S, s> \rightarrow s}\\ % does this fit the general form? This is taken form the book...
\end{mathpar}
...
$\begin{aligned}
    ...
    =\ & \fancybr{\AA}{\mono{x + 1}} s_{1:2:[], \perp}[\mono{x} \mapsto \fancybr{\NN}{\mono{1}}] \mono{:} \fancybr{\CC}{\mono{[x + 1 for x in 2:[] if true]}} s_{1:2:[], \perp}
    ...
    =\ & \mono{2:} \fancybr{\AA}{\mono{x + 1}} s_{1:2:[], \perp}[\mono{x} \mapsto \fancybr{\NN}{\mono{2}}] \mono{:} \fancybr{\CC}{\mono{[x + 1 for x in [] if true]}} s_{1:2:[], \perp}
