...
We are using the same basic form as Nielson \& Nielson \cite[Section 2.1]{wiley} for our semantical rules.
...
These basic rules allow the following semantical rules for our While language:
...
We use this rule to replace the list variable y2 with an actual list literal, which then can be used by the $\CC$ function to compute the resulting list from the list comprehension.
The $\Lstate$ function takes a list variable as input and returns a list literal.
