...
\begin{mathpar}
    \inferrule*[left=$ass$]{}{<x:=a, s> \rightarrow s[x \mapsto \AA \dblbr{a}s]}\\
    \inferrule*[left=$skip$]{}{<skip, s> \rightarrow s}\\
    \inferrule*[left=$comp$]
        {<S1, s> \rightarrow s', <S2, s'> \rightarrow s''}
        {<S1; S2, s> \rightarrow s''} \\
    \inferrule*[left=$if^{tt}$, right={if $\BB \dblbr{b}s = \mathbf{tt}$}]
        {<S1, s> \rightarrow s'}
        {<if\ b\ then\ S1\ else\ S2, s> \rightarrow s'}\\
    \inferrule*[left=$if^{ff}$, right={if $\BB \dblbr{b}s = \mathbf{ff}$}]
        {<S2, s> \rightarrow s'}
        {<if\ b\ then\ S1\ else\ S2, s> \rightarrow s'}\\
    \inferrule*[left=while, right={if $\BB \dblbr{b}s = \mathbf{tt}$}]
        {<S, s> \rightarrow s', <while\ b\ do\ S, s'> \rightarrow s''}
        {<while\ b\ do\ S, s> \rightarrow s''} \\
    \inferrule*[left={$while^{ff}$}, right={if $\BB \dblbr{b}s = \mathbf{ff}$}]
        {}
        {<while\ b\ do\ S, s> \rightarrow s} \\
\end{mathpar}
