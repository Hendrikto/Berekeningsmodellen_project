...
To fix this, we add the function $R$ which replaces the list variables $\in \Lvar$ in a list comprehension back into list literals \metavar{ll}, such that they can be evaluated by the $\CC$ function.
This $R$ function works like as follows:

$R: \Lcomp \text{(with Lvar)} \rightarrow \Lcomp \text{(without Lvar)}$

$\begin{aligned}
    \fancybr{R}{[\text{a for x in y}]} &= \fancybr{R}{[\text{a for x in y if true}]}\\
    \fancybr{R}{[\text{a for x in y if b}]} &= \fancybr{R}{[\text{a for x in } \Lstate(y) \text{ if b}]}\\
\end{aligned}$

\subsubsection{Semantics}

We are using the same basic form as Nielson \& Nielson \cite[Section 2.1]{wiley} for our semantic rules.
This basic rule looks like the following:
\begin{mathpar}
    \inferrule*[right={if \dots}]
        {<S1, s_0> \rightarrow s_1, \dots, <Sn, s_{n-1}> \rightarrow s_n}
        {<S, s_0> \rightarrow s_n}\\
\end{mathpar}

In addition to that, we also have a basic form for the transitions (e.g. for [SKIP] or [$ASS_a$]).
This basic transition is of the following form:
\begin{mathpar}
    \inferrule*[]
        {}
        {<S, s> \rightarrow s'}\\
...
These basic rules allow the following semantic rules for our While language:
...
$s_{x, y}$ means that \mono{l} has value $x$, and \mono{y} has value $y$.
...
\begin{mathpar}
    \inferrule*[right={$[\text{comp}]$}]
        {\inferrule*[right={$[\text{ass}_{ll}]$}]
            {}
            {<\mono{l := 1:2:[]}, s_{\perp, \perp}> \rightarrow s_{1:2:[], \perp}}
        \inferrule*[right={$[\text{lcomp}_{var}]$}]
            {\inferrule*[right={$[\text{ass}_{lc}]$}]
                {\inferrule*[right={$[\text{ass}_{ll}]$}]
                    {}
                    {<\mono{y := 2:3:[]}, s_{1:2:[], \perp}> \rightarrow s_{1:2:[], 2:3:[]}}
                }
                {<\mono{y := [x + 1 for x in 1:2:[]]}, s_{1:2:[], \perp}> \rightarrow s_{1:2:[], 2:3:[]}}
            }
            {<\mono{y := [x + 1 for x in l]}, s_{1:2:[], \perp}> \rightarrow s_{1:2:[], 2:3:[]}}
        }
        {<\mono{l := 1:2:[]; y := [x + 1 for x in l]}, s_{\perp, \perp}> \rightarrow s_{1:2:[], 2:3:[]}}
\end{mathpar}

Because

$\begin{aligned}
    & \fancybr{\CC}{\mono{[x + 1 for x in 1:2:[]]}} s_{1:2:[], \perp}\\
    =\ & \fancybr{\CC}{\mono{[x + 1 for x in 1:2:[] if true]}} s_{1:2:[], \perp}\\
    =\ & \fancybr{\AA}{\mono{x + 1}} s_{1:2:[], \perp}[\mono{x} \mapsto \fancybr{\NN}{\mono{1}}] : \fancybr{\CC}{\mono{[x + 1 for x in 2:[] if true]}} s_{1:2:[], \perp}
    & \text{because } \fancybr{\BB}{\mono{true}} s_{1:2:[], \perp}[\mono{x} \mapsto \fancybr{\NN}{\mono{1}}] = \mathbf{tt}\\
    =\ & \mono{2:} \fancybr{\CC}{\mono{[x + 1 for x in 2:[] if true]}} s_{1:2:[], \perp}\\
    =\ & \mono{2:} \fancybr{\AA}{\mono{x + 1}} s_{1:2:[], \perp}[\mono{x} \mapsto \fancybr{\NN}{\mono{2}}] : \fancybr{\CC}{\mono{[x + 1 for x in [] if true]}} s_{1:2:[], \perp}
    & \text{because } \fancybr{\BB}{\mono{true}} s_{1:2:[], \perp}[\mono{x} \mapsto \fancybr{\NN}{\mono{2}}] = \mathbf{tt}\\
    =\ & \mono{2:3:} \fancybr{\CC}{\mono{[x + 1 for x in [] if true]}} s_{1:2:[], \perp}\\
    =\ & \mono{2:3:[]}\\
\end{aligned}$
...
\begin{itemize}
    \item $S1 \equiv \mono{x := 1:2:3:[]}$
    \item $S2 \equiv \mono{y := [a * 2 for a in x if } b \mono{]}$
    \item $S2' \equiv \mono{y := [a * 2 for a in 1:2:3:[] if } b \mono{]}$
    ...
    \inferrule*[right={$[\text{comp}]$}]
        ...
        \inferrule*[right={$[\text{lcomp}_{var}]$}]
            {\inferrule*[right={$[\text{ass}_{lc}]$}]
                ...
                {<S2', s_1> \rightarrow s_2}
            }
            {<S2, s_1> \rightarrow s_2}
        }
        {<S1 \mono{;} S2, s_0> \rightarrow s_2}
\end{mathpar}
