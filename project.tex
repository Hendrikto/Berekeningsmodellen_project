...
$\begin{aligned}
    \CC \dblbr{[\text{a for x in l}]} s &= \CC \dblbr{[\text{a for x in l if true}]} s\\
    \CC \dblbr{[\text{a for x in [] if b}]} s &= []\\
    \CC \dblbr{[\text{a for x in h:t if b}]} s &= \begin{cases}
        \AA \dblbr{a} s[x \mapsto h] : \CC \dblbr{[\text{a for x in t if b}]} s
        & \text{if } \BB \dblbr{b} s[x \mapsto h] = \mathbf{tt}\\
        \CC \dblbr{[\text{a for x in t if b}]} s
        & \text{if } \BB \dblbr{b} s[x \mapsto h] = \mathbf{ff}\\
    \end{cases}\\
\end{aligned}$
...
\begin{mathpar}
    \inferrule*[left={$[\text{ass}_a]$}]
        {}
        {ENV \vdash <\text{x := a}, s> \rightarrow ENV \vdash s[x \mapsto \fancybr{\AA}{a}s]}\\
    \inferrule*[left={$[\text{ass}_{lc}]$}]
        {ENV \vdash <l, s> \rightarrow ENV' \vdash s}
        {ENV \vdash <\text{y := l}, s> \rightarrow [] \vdash s[y \mapsto \fancybr{\LL}{ENV'}]}\\
    \inferrule*[left={$[\text{ass}_{ll}]$}]
        {}
        {ENV \vdash <\text{y := l}, s> \rightarrow ENV \vdash s[y \mapsto \fancybr{\LL}{l}]}\\
    ...
    \inferrule*[left={$[\text{lcomp}^{var}]$}]
        {ENV \vdash <\text{[a for x in } \Lstate(y)], s> \rightarrow ENV \vdash s}
        {ENV \vdash <\text{[a for x in y]}, s> \rightarrow ENV \vdash s}\\
    \inferrule*[left={$[\text{lcomp}^{tt\_start}]$}, right={if $\fancybr{\BB}{b}s[x \mapsto \fancybr{\NN}{h}] = \mathbf{tt}$}]
        {\fancybr{\AA}{a}s[x \mapsto h]:[] \vdash <\text{[a for x in t if b]}, s> \rightarrow ENV \vdash s}
        {[] \vdash <\text{[a for x in h:t if b]},s> \rightarrow ENV \vdash s}\\
    \inferrule*[left={$[\text{lcomp}^{tt\_mid}]$}, right={if $\fancybr{\BB}{b}s[x \mapsto \fancybr{\NN}{h}] = \mathbf{tt}$}]
        {head:\fancybr{\AA}{a}s[x \mapsto h]:[] \vdash <\text{[a for x in t if b]}, s> \rightarrow ENV \vdash s}
        {head:[]  \vdash <\text{[a for x in h:t if b]},s> \rightarrow ENV \vdash s}\\
    \inferrule*[left={$[\text{lcomp}^{ff}]$}, right={if $\fancybr{\BB}{b}s[x \mapsto \fancybr{\NN}{h}] = \mathbf{ff}$}]
        {ENV \vdash <\text{[a for x in t:[] if b]}, s> \rightarrow ENV' \vdash s'}
        {ENV \vdash <\text{[a for x in h:t:[] if b]},s> \rightarrow ENV' \vdash s'}\\
\end{mathpar}
...
\begin{mathpar}
    \inferrule*[right={$[\text{comp}]$}]
        ...
        {[] \vdash <\text{l := 1:2:[]; y := [x + 1 for x in l]}, s_{\perp, \perp}> \rightarrow [] \vdash s_{1:2:[], 2:3:[]}}
\end{mathpar}
\end{landscape}
