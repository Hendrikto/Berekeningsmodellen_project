...
\begin{mathpar}
    \inferrule*[right={if \dots}]
        {<S1, s> \rightarrow s1', \dots, <Sn, sn> \rightarrow s'}
        {<S, s> \rightarrow s'}\\
\end{mathpar}
...
When the list matches \mono{[]}, which means it is empty, we have to use our last rule $[lcomp^{end}]$, which looks as follows:
